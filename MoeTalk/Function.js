...
if(!localStorage['mt-name'])localStorage['mt-name'] = '{}';var mt_name = JSON.parse(localStorage['mt-name']);//默认样式
if(localStorage['qchar'])
{
...
{
	let imglink = id+'/'+img
	if(isNaN(parseInt(id)))imglink = mt_characters[id].school+'/'+mt_characters[id].club+'/'+imglink
	if(JSON.parse(localStorage['heads'])[0] && JSON.parse(localStorage['heads'])[0][imglink])
	{//自定义头像
		return JSON.parse(localStorage['heads'])[0][imglink];
	}
	else
	{//本地头像
		let image = 'char/';
		image = image+imglink+'.webp';
		// if(sessionStorage[img])image = 'data:image/webp;base64,'+sessionStorage[img];
		// if(!sessionStorage[img] && localStorage['imgs'] == 851)
		// {
		// 	let db;
		// 	openDB('MoeTalk').then((db =>
		// 	{
		// 		db = db;
		// 		getDataByKey(db,'Custom',img).then((arr =>
		// 		{
		// 			if(arr)
		// 			{
		// 				sessionStorage[img] = arr['val'];
